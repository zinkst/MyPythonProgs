...
###########################################################################
def extendInputParams(inputParams, year, fileType):
  ...
  inputParams["FILE_TYPE"] = fileType
...
def processFileObjects(fileObjects, inputParams):
  ...
  with open(os.path.join(outFilePath, "FoundFiles.txt"), 'w') as outfile:
    for fileObject in fileObjects:
      ...
      createSymlinkForFileObject(newTgtDir, fileObject)
      outLine = fileObject.findMethod.rjust(16) + ":" + fileObject.fileBaseName.rjust(60) + " == " + os.path.basename(
        fileObject.absDateiNameOnOriginal) + " ## " + fileObject.absCopiesOrigDateiName + " == " + fileObject.absDateiNameOnOriginal
...
def createSymlinkForFileObject(newTgtDir, fileObject):
  try:
...
inputParams = extendInputParams(inputParams, args.year, args.type)
